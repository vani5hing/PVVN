...
Bài này mình làm cách khác guyintuxedo, vì `tcache` chỉ xóa mỗi `key` chứ không xóa data nên `ptr` vẫn còn đó khi `malloc` lại. Lợi dụng cái này để leak heap (thông qua chunk cuối trong `tcache` sẽ có `foward_ptr = 0 ^ current_ptr >> 12`). Sau đó abuse `stack/pie` leak qua hàm `secret` để `tcache poisoning` lên `stack`. Edit `saved rip` về `win`. POC:

(tự compile code ra file thực thi trên local nhe) (this was compiled in WSL2 ubuntu 22.04.05)
...
mangled = (rbp_secret - 0x10) ^ (heap_base + 0x16f0) >> 12
edit(1, p64(mangled))
